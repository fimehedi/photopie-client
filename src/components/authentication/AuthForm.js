...
const AuthForm = ({ isSignup }) => {
	...
	if (loggedInUser.isAuthenticated) navigate(from);
	...
	const handleOnChange = (e) => {
		...
		if (e.target.name === 'name') {
			...
			const passHasNumberAndChar = /^(?=.*\d)(?=.*[a-zA-Z]).{6,16}$/.test(
				password
			);
	...
	const handleOnSubmit = (e) => {
		...
		if (isSignup && (!name || !email || !confirmPassword)) {
			...
			setStatus({
				isError: true,
				errorMsg: 'Password required at least one Numeric and one Character!',
			});
	...
	return (
		<div className="min-h-screen bg-gray-100 flex flex-col justify-center p-5">
			<div className="xs:p-0 mx-auto md:w-full md:max-w-md">
				<Link to="/">
					<h1 className="text-center mb-5 font-semibold text-xl tracking-tight cursor-pointer">
						PHOTOPIE
					</h1>
				</Link>
				<div className="bg-white shadow w-full rounded-lg divide-y divide-gray-200">
					<div className="px-5 py-7">
						...
						<form onSubmit={handleOnSubmit}>
							...
							{status.isError && (
								<small className="text-red-500">{status.errorMsg}</small>
							)}
							{status.isSuccess && (
								<small className="text-green-500">{status.successMsg}</small>
							)}
							<button
								type="submit"
